...
### Size Filtering (Decks)
- Deck width is automatically extracted from product names (e.g., 8.25", 8.0")
- Size filter buttons in the HTML report to filter by deck width
- Supported sizes: 7.5" to 10.5"

### Price History Tracking
- Daily prices are tracked in price_history.json
- "LOWEST" badge appears when current price is the lowest ever tracked
- Trend indicators show if price is rising (↑) or falling (↓)
- Shows how many days an item has been tracked
...
├── price_history.json   # Price history for trend tracking
...
- **2024-12-03**: Fixed "Recent Changes" section - now always visible, shows "No changes detected" when empty
